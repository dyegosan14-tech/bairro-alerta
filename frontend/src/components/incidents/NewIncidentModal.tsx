      ...
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-brand-600 to-sky-600 text-white">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-base">Registrar Nova Ocorrência Urbana</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4 text-xs">
          ...
          {/* Categoria */}
          <div>
            <label className="block font-bold text-slate-700 mb-1.5">Selecione a Categoria do Problema *</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block font-bold text-slate-700 mb-1">Logradouro / Endereço Aproximado *</label>
              <input
                type="text"
                required
                value={address}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-start">
            <div>
              <label className="block font-bold text-slate-700 mb-1">Gravidade / Urgência</label>
              <select
                value={priority}
            ...
            <div>
              <label className="block font-bold text-slate-700 mb-1">Foto da Ocorrência (Opcional)</label>
              <div className="flex items-center space-x-2">
                <label className="flex-1 cursor-pointer flex items-center justify-center space-x-2 bg-slate-50 hover:bg-slate-100 border border-dashed border-slate-300 rounded-xl px-3 py-2 text-slate-600 transition-colors">
                  ...
                  <span className="truncate">{selectedFile ? selectedFile.name : 'Escolher foto...'}</span>
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-end space-x-2">
            ...
            >
              ...
              <span>{isSubmitting ? 'Enviando Ocorrência...' : 'Publicar Ocorrência'}</span>
            </button>
          </div>
