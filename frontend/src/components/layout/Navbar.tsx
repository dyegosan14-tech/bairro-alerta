    ...
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Logo e Nome */}
          <div className="flex items-center space-x-3 cursor-pointer" onClick={() => onSelectTab('map')}>
              ...
              <div className="flex items-center space-x-2 pl-2 border-l border-slate-200">
                <div className="hidden lg:flex flex-col text-right">
                  ...
                  <span className={`text-[10px] font-bold uppercase tracking-wider ${
                    user.role === 'ADMIN' ? 'text-purple-600' : user.role === 'MODERATOR' ? 'text-amber-600' : 'text-brand-600'
                  }`}>
                    {user.role === 'ADMIN' ? 'Admin' : user.role === 'MODERATOR' ? 'Moderador' : 'Cidadão'}
                  </span>
                </div>
                <button
                  onClick={logout}
