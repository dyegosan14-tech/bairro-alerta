      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500">
            Registro criptográfico e imutável de todas as ações administrativas, alterações de dados e moderações.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
          <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <span className="font-bold text-xs text-slate-700 uppercase tracking-wider">Eventos do Sistema</span>
            ...
            {selectedLog ? (
              <div className="space-y-4 mt-4 overflow-y-auto max-h-[480px]">
                {selectedLog.old_values && (
                  <div>
                    <span className="text-rose-400 font-bold block mb-1 text-[11px]">- Estado Anterior (old_values):</span>
                ...
                {selectedLog.new_values && (
                  <div>
                    <span className="text-emerald-400 font-bold block mb-1 text-[11px]">+ Novo Estado (new_values):</span>
                ...
                {!selectedLog.old_values && !selectedLog.new_values && (
                  <p className="text-slate-500 italic">Nenhum payload de alteração de estado para este evento.</p>
                )}
              </div>
            ) : (
              <div className="py-24 text-center text-slate-500">
                <Database className="w-8 h-8 mx-auto mb-2 opacity-50" />
                <p>Selecione um log ao lado para inspecionar os deltas e alterações brutas de dados.</p>
              </div>
            )}
          </div>
